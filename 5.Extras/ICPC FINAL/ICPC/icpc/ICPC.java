...
/**
 * Crea un red ICPC donde se pueden agregar y modificar intersecciones, rutas y sennales.
 * 
...
public class ICPC
{
    ...
    public ICPC(int cost, int[][] routesSpeedLimits) {
        this.ok = false;
        if(varificaValoresMatriz(routesSpeedLimits)){
            if(cost > 0 ){
                this.cost = cost;
            }
            Canvas board = Canvas.getCanvas("ICPC",800,800);
            board.redraw1();
    ...
    public void addIntersection(String type, String color, int x, int y){
        ...
        if(!intersecciones.containsKey(color) && !verificarPosInterseccion(x,y) && typeIntersectionExist(type)){
            ...
            if(type.equals("hermit")){
                intersecciones.put(color,new Hermit(color,x,y));
            }
            if(type.equals("needy")){
                intersecciones.put(color,new Needy(color,x,y));
            }
            this.ok = true;
            if (this.isVisible) {
                Interseccion interseccion = intersecciones.get(color);
     ...
     */
    protected String numberColor(int numero){
    ...
    /**
     * Metodo que modifica el costo de la red ICPC.
     * @param int costo, costo minimo de la red ICPC.
     */
    protected void setCost(int costo){
        cost = costo;
    }
    ...
    private boolean ifIsHermit(String interseccionA, String interseccionB){
        ...
        if(intersectionA.getType().equals("hermit") && intersectionA.getSize() >= 1){
            bandera = false;
        }
        else if(intersectionB.getType().equals("hermit") && intersectionB.getSize() >= 1){
            bandera = false;
        }
        return bandera;
    }
    ...
    private boolean ifIsFixedInterseccion(String color){
        boolean bandera = false;
        Interseccion interseccion = intersecciones.get(color);
        ArrayList<Ruta> routes = interseccion.getRutas();
    ...
    /*
     * Metodo que verifica que los numeros existan
     */
    private boolean varificaValoresMatriz(int[][] numeros){
        boolean bandera = true;
        int i = 0;
        while(bandera && i < numeros.length){
            int j = 0;
            while(bandera && j <= 1){
                if(numeros[i][j] >= 6 || numeros[i][j] < 0){
                    bandera = false;
                }
                j ++;
            }
